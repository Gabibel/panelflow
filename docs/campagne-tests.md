...
**Lire le registre.** `challenge` (un mur Cloudflare) et `timeout` depuis ce PC ne condamnent pas un site : l'antivirus de cette machine bloque un tiers des domaines, et un téléphone sur un autre réseau les voit. `moved` et `error:ENOTFOUND` sont des entrées à retirer ou à corriger. `no-sample` veut dire que ni la page d'accueil, ni la première page de série, ni `docs/sites-samples.json` ne donnent de chapitre : le site est vivant mais la preuve de lecture manque, et c'est à faire à la main (§4.2). `hand:…` est ce qu'une personne a vu en ouvrant le site dans un navigateur ; la passe du 18 septembre a ainsi sorti 31 domaines morts des règles et en a renommé 6.
...
**4.3 Les candidats : deux cents sites pris dans les index communautaires (21 septembre).** `docs/sites-candidates.json` liste cinquante sites d'anime, cinquante de manga, cinquante de light novels et cinquante de webtoons, pris sur fmhy.net (readingpiracyguide, videopiracyguide) et wotaku.wiki, plus les sites français déjà dans les règles. `node scripts/check-sites.mjs --candidates` les visite tous et écrit `docs/sites-candidates-results.json` et `docs/sites-candidates.md` (une colonne Script, une colonne Navigateur). Le script voit depuis ce PC ce que voit un robot : sur les sites de lecture, il cherche un chapitre et compte ses images ou ses paragraphes ; sur les sites de vidéo, il cherche un épisode et un lecteur (`<video>`, `<iframe>` d'un hébergeur listé, sélecteur d'épisodes). Ce que le script ne peut pas voir, une personne l'a ouvert dans un navigateur, avec une sonde en deux temps (trouver un épisode ou un chapitre depuis l'accueil, puis mesurer la page) : 71 sites ainsi, dont tous les sites d'anime en JavaScript.

Ce que la passe a donné, par famille :

- **Anime (51)** : 14 sites dont la page d'épisode porte un lecteur (kaa.lt, animex.one, animesuge.cz, anistream.one, nekowatch.xyz, justanime.to, anilight.live, anikuro.to, senshi.to, animeparadise.moe, yomi.to, anihq.cc, voiranime.rip, franime.fr) ; leurs hébergeurs sont entrés dans `videoDomains` (krussdomi.com, megaplay.buzz, aniembed.se, theanimecommunity.com, embed.yaply.net, voe.sx et les sites eux-mêmes). Sept sont en panne (la famille aniwatchtv/gogoanimez/anivibe/hianime.ad/anitaku répond une erreur de base de données), trois derrière Turnstile, onze sont des applications JavaScript sans aucun lien d'épisode dans le document (miruro, mkissa, allmanga, anidap, kazora, anizone, anidoor, anime.nexus, meguanime, kawaiianime, reanime) : sur ceux-là, seule une visite au téléphone dira si la page d'épisode finit par porter un lecteur. Deux navigations refusées par le navigateur intégré, une page qui ferme l'onglet (lunarx.to).
- **Manga (47)** : prêts au script ou au navigateur : manganato.gg, mangakatana.com, mangapill.com, weebcentral.com, mangafire.to, lelmanga.com, mangas-origines.fr, comizy.io (l'adresse actuelle de mangabuddy), asurascans.com (celle d'asuracomic), likemanga.ink, mangaberri.com, mgeko.cc. Deux sites sont paginés, une image par adresse (mangago.me, scan-vf.net) : le mode page à page du 21 septembre les lit (§4.4). Murs Cloudflare : sushiscan.net, japscan.foo, kagane.to, comix.to, mangakakalot.gg sur la page de chapitre. Morts : mangapark.net (redirige vers une régie), scan-manga.com (404), mangareader.to (522), reaper-scans.fr (document vide).
- **Light novels (50)** : le texte est reconnu là où la page répond : webnovel.com (75 paragraphes), royalroad.com, novelcool.com, kakuyomu.jp, readnovelfull.com, novelfull.com, freewebnovel.com, wuxiaworld.com. Onze sites répondent 403 à ce PC, neuf ont un mur, dix ne répondent pas à temps : c'est la famille la moins observable d'ici, et la campagne au téléphone (§4.2) doit commencer par elle. chireads.com et xiaowaz.fr (français) sont entrés dans les règles en `novel`.
- **Webtoons (50)** : webtoons.com (103 images), asurascans.com, comizy.io, rizzfables.com, flamecomics.xyz ; kingofshojo.com et manhwabuddy.com entrés en `webtoon`. Sept domaines ont déménagé (`moved`), quatorze ne répondent pas à temps, sept répondent 403 : les entrées `moved` sont à renommer à la prochaine passe.

**4.4 Le mode page à page (21 septembre).** Deux formes, dans `extension/content/detect.js`. scan-vf écrit les dix-sept pages du chapitre dans le document, en `<img data-src>` dans un conteneur caché, et n'en affiche qu'une : `parkedStrip()` lit le chapitre par adresse (trois images ou plus classées dans le même dossier), sans un seul fetch. mangago n'écrit aucune image dans le balisage (son script les décode une fois la page lancée) et offre un lien « next > » vers `/pg-2/` et `total_pages=48` : `pagedByNext()` déduit la forme des adresses du lien (`pageTemplate`, qui refuse un lien de chapitre suivant), le compte de la page, et `walkPages()` lit chaque page, par le balisage quand il porte l'image, sinon par une frame cachée de la page elle-même (`panelInFrame`, sandbox sans pop-up ni navigation, quatre à la fois). Le lecteur s'ouvre sur les trois premières pages et se remplit (`PanelFlowReader.addPages`) : six pages en quatre secondes sur le vrai mangago. Sans compte de pages, la marche suit « next » jusqu'à ce que le lien quitte le chapitre. Tests : `backend/test/page-by-page.test.js` (les fonctions réelles sur le balisage réduit des deux sites) et la campagne longue ci-dessous.

**4.5 La campagne longue (`backend/test/e2e/long-run.e2e.test.js`).** Un seul profil de navigateur, l'extension réelle, cinq sites synthétiques servis par `fixtures.js` sous leurs vrais domaines (mangakakalot.gg, scan-vf.net, mangago.me, readnovelfull.com, voiranime.rip avec un lecteur sibnet.ru en frame) : une personne qui ajoute un manga depuis le signet du lecteur, le lit jusqu'au bout, revient le lendemain, enchaîne les chapitres sans quitter le lecteur, ouvre la roue des chapitres, garde un chapitre hors ligne et le relit sans le site, lit un light novel, ajoute un anime depuis la barre du lecteur vidéo, ouvre les deux sites paginés, puis vérifie ce que la bibliothèque, l'historique et les statistiques ont retenu. Chaque geste est celui d'une main (une page ouverte, un bouton, une touche, un défilement) et chaque vérification est posée à l'extension comme son propre popup la pose. La première exécution a trouvé deux défauts de long terme : le chrono de lecture n'était pas mis en banque au changement de chapitre en place (dix chapitres lus d'affilée donnaient une seule ligne d'historique, sur le dixième), et une série était classée sous deux adresses selon que la page de chapitre liait ou non la page de série (avec ou sans barre finale), si bien que Continuer et la roue ignoraient les chapitres lus depuis la seconde (`filedUnder` dans `shared/panelflow-core.js`, `backend/test/filed-under.test.js`). Elle tourne dans la CI (job `e2e`) avec `extension.e2e.test.js`.

Deux règles de reconnaissance sont sorties de cette passe, dans `extension/content/video-speed.js` et `detect.js` : une page dont l'adresse nomme un épisode (chemin `/episode-3/`, `/ep-34`, ou requête `?ep=12` comme franime et anilight) et qui porte une `<iframe>` d'un autre site est une page d'épisode, même si l'hébergeur n'est pas listé ; les frames de publicité et de commentaires sont nommées et exclues. Et la bibliothèque marque ce qu'elle tient : sur une œuvre déjà enregistrée depuis le même site, le signet du lecteur, celui de la barre vidéo (jusque dans la frame du lecteur) et le bouton Ajouter du téléphone portent une petite croix (`shared/series-match.js`, `onThisSite`).
